...
def card(children, style=None):
    base = {"background": CARD, "borderRadius": "12px", "padding": "18px", "border": f"1px solid {BORDER}", "height": "100%"}
...
app.layout = dbc.Container([
    # Top Header
    dbc.Row([
        dbc.Col(html.Div("RegAlert — MedTech Regulatory & Research Monitor", 
                         style={"fontSize": "26px", "fontWeight": "600", "color": ACCENT}), xs=12, lg=8),
        dbc.Col(html.Div(id="alert-indicator", style={"textAlign": "right"}), xs=12, lg=4)
    ], className="g-0 mt-4 mb-4 align-items-center"),
    ...
    dbc.Row([
        ...
        dbc.Col([
            # KPI Row
            dbc.Row([
                dbc.Col(card([html.Div("TOTAL RECORDS", style={"color": MUTED, "fontSize": "11px"}), html.Div(id="kpi-total", style={"fontSize": "26px", "color": ACCENT, "fontWeight": "700"})]), xs=6, md=3, className="mb-3"),
                dbc.Col(card([html.Div("SCIENTIFIC", style={"color": MUTED, "fontSize": "11px"}), html.Div(id="kpi-scientific", style={"fontSize": "26px", "color": ACCENT, "fontWeight": "700"})]), xs=6, md=3, className="mb-3"),
                dbc.Col(card([html.Div("REGULATORY", style={"color": MUTED, "fontSize": "11px"}), html.Div(id="kpi-regulatory", style={"fontSize": "26px", "color": ACCENT, "fontWeight": "700"})]), xs=6, md=3, className="mb-3"),
                dbc.Col(card([html.Div("LATEST DATA", style={"color": MUTED, "fontSize": "11px"}), html.Div(id="kpi-latest-year", style={"fontSize": "26px", "color": ACCENT, "fontWeight": "700"})]), xs=6, md=3, className="mb-3"),
            ]),
            
            # Watchlist & Trends Row
            dbc.Row([
                dbc.Col(card([
            ...
            # Full Width Chart - Cumulative
            dbc.Row([
                dbc.Col(card([html.Div("Cumulative Activity Over Time", style=TITLE_STYLE), dcc.Graph(id="chart-time", style={"height": "300px"})]), xs=12, className="mb-3"),
            ]),

            # FIXED: Evidence Gap Analysis (Full width for better readability)
            dbc.Row([
                dbc.Col(card([html.Div("Evidence Gap Analysis (Research vs Regulatory)", style=TITLE_STYLE), dcc.Graph(id="chart-gap", className="gap-chart-rounded", style={"height": "400px"})]), xs=12, className="mb-3"),
            ]),

            # FIXED: Domain Trends (Full width to prevent legend overlap)
            dbc.Row([
                dbc.Col(card([html.Div("Domain Trends (Quarterly)", style=TITLE_STYLE), dcc.Graph(id="chart-trends", style={"height": "400px"})]), xs=12, className="mb-3"),
            ]),

            # Activity by Source & Recall Radar (Side by side is OK for these)
            dbc.Row([
                dbc.Col(card([html.Div("Activity by Source", style=TITLE_STYLE), dcc.Graph(id="chart-source", style={"height": "350px"})]), xs=12, lg=6, className="mb-3"),
                dbc.Col(card([html.Div("Recall Radar", style=TITLE_STYLE), dcc.Graph(id="chart-recall-causes", style={"height": "350px"})]), xs=12, lg=6, className="mb-3"),
            ]),
            
            # Concentration Bar Chart
            dbc.Row([
                dbc.Col(card([html.Div("Top Domains Concentration", style=TITLE_STYLE), dcc.Graph(id="chart-top-domains", className="domains-chart-rounded", style={"height": "400px"})]), xs=12, className="mb-3"),
            ]),
...
], fluid=True, style={"backgroundColor": BG, "padding": "25px", "minHeight": "100vh"})
...
def update_dashboard(sources, categories, years, n_clicks, search_text, ai_mode):
    ...
    fig_source.update_traces(marker=dict(line=dict(color=CARD, width=2)), opacity=0.65, textinfo='percent', textposition='outside')
    ...
    fig_top.update_traces(marker_color=ACCENT, marker_opacity=0.6, marker_line_color=ACCENT, marker_line_width=1.5)
